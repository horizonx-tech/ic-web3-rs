...
impl<T: Transport> ParityAccounts<T> {
    ...
    pub fn parity_kill_account(&self, address: &Address, pwd: &str, options: CallOptions) -> CallFuture<bool, T::Out> {
        ...
        CallFuture::new(
            self.transport
                .execute("parity_killAccount", vec![address, pwd], options),
        )
    }
    ...
    pub fn parity_new_account_from_phrase(
        ...
        CallFuture::new(
            self.transport
                .execute("parity_newAccountFromPhrase", vec![seed, pwd], options),
        )
    }
    ...
    pub fn new_account_from_secret(
        ...
        CallFuture::new(
            self.transport
                .execute("parity_newAccountFromSecret", vec![secret, pwd], options),
        )
    }
    ...
    pub fn parity_new_account_from_wallet(
        ...
        CallFuture::new(
            self.transport
                .execute("parity_newAccountFromWallet", vec![wallet, pwd], options),
        )
    }
    ...
    pub fn parity_remove_address(&self, address: &Address, options: CallOptions) -> CallFuture<bool, T::Out> {
        let address = helpers::serialize(&address);
        CallFuture::new(self.transport.execute("parity_removeAddress", vec![address], options))
    }
}
...
mod tests {
    ...
    use crate::{api::Namespace, rpc::Value, transports::ic_http_client::CallOptions};
    use ethereum_types::{Address, H256};

    rpc_test! (
        ParityAccounts :   parity_kill_account,  &"9b776baeaf3896657a9ba0db5564623b3e0173e0".parse::<Address>().unwrap(), "123456789",CallOptions::default()
        => "parity_killAccount", vec![r#""0x9b776baeaf3896657a9ba0db5564623b3e0173e0""#, r#""123456789""#];
        Value::Bool(true) => true
    );

    rpc_test! (
        ParityAccounts :   parity_new_account_from_phrase,  "member funny cloth wrist ugly water tuition always fall recycle maze long", "123456789",CallOptions::default()
    ...
    rpc_test! (
        ParityAccounts :   new_account_from_secret,  &"c6592108cc3577f6a2d6178bc6947b43db39057195802caa0120f26e39af4945".parse::<H256>().unwrap(), "123456789",CallOptions::default()
    ...
    rpc_test! (
        ParityAccounts :   parity_remove_address,  &"9b776baeaf3896657a9ba0db5564623b3e0173e0".parse::<Address>().unwrap(),CallOptions::default()
